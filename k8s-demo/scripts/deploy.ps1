...
if ($Force) {
    ...
    # Recuperer les fichiers modifies depuis le dernier commit
    ...
    $k8sChanged      = ($changed | Where-Object { $_ -like "k8s/*"      }).Count -gt 0
}
...
if ($k8sChanged) {
    ...
    Write-Host "[K8S] Manifests appliques" -ForegroundColor Green
}
...
if ($backendChanged) {
    ...
    # Vider la collection products pour forcer le re-seed
    ...
    if ($LASTEXITCODE -ne 0) { Write-Host "ERREUR : rechargement MongoDB echoue" -ForegroundColor Red; exit 1 }

    # 2e restart pour declencher le re-seed automatique de Flask
    kubectl rollout restart deployment/backend -n $NAMESPACE
    kubectl rollout status  deployment/backend -n $NAMESPACE --timeout=120s
    Write-Host "[MONGODB] Base de donnees rechargee avec les nouvelles donnees" -ForegroundColor Green
}

# ── 6. Deploiement Frontend ──────────────────────────────────────
if ($frontendChanged) {
